...
limit_bandwidth(){
	...
	tc qdisc add dev $DHCPIF root handle 1: htb
        tc class add dev $DHCPIF parent 1: classid 1:1 htb rate 1Gbit

        ip link add dev ${DHCPIF}-ifb name ${DHCPIF}-ifb type ifb
        ip link set dev ${DHCPIF}-ifb up

        tc qdisc add dev ${DHCPIF}-ifb root handle 1: htb
        tc class add dev ${DHCPIF}-ifb parent 1: classid 1:1 htb rate 1Gbit

        tc qdisc add dev $DHCPIF ingress
        tc filter add dev $DHCPIF parent ffff: protocol ip u32 match u32 0 0 flowid 1:1 action mirred egress redirect dev ${DHCPIF}-ifb
...
run_up() {
    ...
    if iptables -C INPUT -p tcp --dport 3990 -j ACCEPT 2>/dev/null; then
        logger "iptables rule for port 3990 already added"                         
        logger "port: $UAMPORT"                                                                                                        
    else                                            
        # Add the rule if it doesn't exist
        ipt -I INPUT -p tcp --dport 3990 -j ACCEPT
    fi 
