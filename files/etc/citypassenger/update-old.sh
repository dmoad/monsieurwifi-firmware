...
if [ "$status" = "success" ]; then
    ...
    # Commit changes if any SSID, password, visibility, radio setting, or MAC filter has changed
    if [ "$ssid_changed" = true ] || [ "$password_changed" = true ] || [ "$visibility_changed" = true ] || [ "$radio_changed" = true ] || [ "$mac_filter_changed" = true ]; then
        echo "Wireless configuration changed, committing changes and restarting WiFi..."
        uci commit wireless
    ...
    if [ "$chilli_ip" != "" ] && [ "$chilli_ip" != "null" ]; then
        if [ "$current_chilli_ip" != "$chilli_ip" ]; then
            ...
            if [ "$vlan_enabled" = "true" ]; then
                ...
                local effective_captive_vlan="$captive_portal_vlan"
    ...
    if [ "$mac_filter_changed" = true ]; then
        uci commit wireless
        wifi reload
        echo "MAC filtering updated and wireless restarted"
    fi
fi
