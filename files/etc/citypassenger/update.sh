...
if [ "$status" = "success" ]; then
    ...
    echo "Current blocked domains after xargs: $current_blocked_domains"
    ...
    if [ -n "$blocked_domains_json" ] && [ "$blocked_domains_json" != "null" ] && [ "$blocked_domains_json" != "[]" ]; then
        ...
        if [ -n "$current_blocked_domains" ]; then
            ...
            for domain in $current_blocked_domains; do
                echo "Removing blocked domain because no blocked domains in API: $domain"
                if [ -n "$domain" ]; then
                    uci del_list dhcp.@dnsmasq[0].address="/$domain/$captive_portal_ip"
                    echo "Removed blocked domain: $domain"
                fi
    ...
    echo "Current blocked domains after processing: $current_blocked_domains"
    ...
    # Handle DNS filtering service based on blocked domains configuration
    dns_filtering_service="/etc/init.d/dns_filtering"
    dns_redirect_flag="/tmp/dns_redirect_enabled"

    # Check current DNS filtering service status
    dns_filtering_currently_enabled=false
    if [ -f "$dns_redirect_flag" ]; then
        dns_filtering_currently_enabled=true
    fi

    echo "DNS filtering service status check:"
    echo "  Has blocked domains: $has_blocked_domains"
    echo "  DNS filtering currently enabled: $dns_filtering_currently_enabled"

    # Enable DNS filtering service if blocked domains are present and it's not already enabled
    if [ "$has_blocked_domains" = true ] && [ "$dns_filtering_currently_enabled" = false ]; then
        echo "Enabling DNS filtering service due to blocked domains..."
        if [ -f "$dns_filtering_service" ]; then
            "$dns_filtering_service" start
            if [ $? -eq 0 ]; then
            ...
            echo "Warning: DNS filtering service not found at $dns_filtering_service"
        fi
    # Disable DNS filtering service if no blocked domains and it's currently enabled
    elif [ "$has_blocked_domains" = false ] && [ "$dns_filtering_currently_enabled" = true ]; then
        echo "Disabling DNS filtering service as no blocked domains are configured..."
        if [ -f "$dns_filtering_service" ]; then
            "$dns_filtering_service" stop
            if [ $? -eq 0 ]; then
                echo "DNS filtering service stopped successfully"
            else
                echo "Failed to stop DNS filtering service"
            fi
        else
            echo "Warning: DNS filtering service not found at $dns_filtering_service"
        fi
    else
        echo "DNS filtering service status unchanged (appropriate for current configuration)"
    fi

    # Manage DNS filtering service auto-start on boot
    if [ "$has_blocked_domains" = true ]; then
        # Enable service to start on boot when blocked domains are configured
        if [ -f "$dns_filtering_service" ]; then
            "$dns_filtering_service" enable 2>/dev/null
            echo "DNS filtering service enabled for auto-start on boot"
        fi
    else
        # Disable service from starting on boot when no blocked domains
        if [ -f "$dns_filtering_service" ]; then
            "$dns_filtering_service" disable 2>/dev/null
            echo "DNS filtering service disabled from auto-start on boot"
        fi
    fi

    # Show final DNS filtering service status
    if [ -f "$dns_filtering_service" ]; then
        echo "Final DNS filtering service status:"
        "$dns_filtering_service" status 2>/dev/null || echo "  Service status check failed"
    fi
